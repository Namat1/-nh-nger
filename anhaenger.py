...
uploaded_files = st.file_uploader(
    "Lade deine Excel- oder CSV-Dateien hoch",
    type=["xlsx", "xls", "csv"],
...
if uploaded_files:
    ...
    for idx, uploaded_file in enumerate(uploaded_files):
        try:
            ...
            kalenderwoche = (
                f"KW{kw_match.group(1)}" if kw_match else "Keine KW gefunden"
            )
            ...
            if all(col in df.columns for col in required_columns):
                ...
                text_matches = df[
                    df["Unnamed: 14"].str.contains(
                        "|".join(search_strings), case=False, na=False
                    )
                    & (df["Unnamed: 11"] != "607")
                ]
                combined_results_df = pd.concat(
                    [number_matches, text_matches]
                ).drop_duplicates()
                ...
                final_results = combined_results_df[required_columns].rename(
                ...
                final_results["Verdienst"] = final_results.apply(
                    calculate_payment, axis=1
                )

                # Zeilen mit 0 oder NaN in "Verdienst" entfernen
                final_results = final_results[
                    (final_results["Verdienst"] > 0)
                    & final_results["Verdienst"].notna()
                ]
                final_results["Verdienst"] = final_results["Verdienst"].apply(
                    lambda x: f"{x} €"
                )
                ...
                summary = (
                    summary.groupby(["KW", "Nachname", "Vorname"])
                    .agg({"Verdienst": "sum"})
                    .reset_index()
                )
                summary["Gesamtverdienst"] = summary["Verdienst"].apply(
                    lambda x: f"{x} €"
                )
    ...
    if all_results:
        ...
        combined_results["KW_Numeric"] = (
            combined_results["KW"].str.extract(r"(\d+)").fillna(0).astype(int)
        )
        combined_results = combined_results.sort_values(
            by=["KW_Numeric", "Nachname", "Vorname"]
        ).drop(columns=["KW_Numeric"])

        combined_summary = pd.concat(all_summaries, ignore_index=True)
        combined_summary["KW_Numeric"] = (
            combined_summary["KW"].str.extract(r"(\d+)").fillna(0).astype(int)
        )
        ...
        if combined_results is not None and combined_summary is not None:
            ...
            with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
                ...
                combined_results.to_excel(
                    writer, index=False, sheet_name="Suchergebnisse"
                )

                # Blatt 2: Zusammenfassung
                combined_summary.to_excel(
                    writer, index=False, sheet_name="Zusammenfassung"
                )
...
vehicle_grouped["Gesamtsumme (€)"] = vehicle_grouped.iloc[:, 4:].sum(
    axis=1
)  # Summiere alle Fahrzeugspalten

# Optional: Formatierung für Euro-Zeichen
for col in vehicle_grouped.columns[4:]:  # Fahrzeug-Spalten und Gesamtsumme formatieren
...
for col_num, column_name in enumerate(vehicle_grouped.columns):
    max_width = max(
        vehicle_grouped[column_name].astype(str).map(len).max(), len(column_name)
    )
