...
if combined_results is not None and combined_summary is not None:
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        cleaned_results = combined_results.fillna("")  # NaN entfernen
       ...
       # Farben und Zellformatierungen anwenden
       for row_num in range(len(cleaned_results)):
           kw = cleaned_results.iloc[row_num]['KW']
           if kw != current_kw:
               current_kw = kw
           ...
           for col_num, value in enumerate(cleaned_results.iloc[row_num]):
